...
> **Generative AI** is a class of machine learning models that learn the **statistical distribution** of training data and can sample from that distribution to produce entirely new, plausible outputs.
> Unlike **discriminative models** (which classify existing data), generative models **create** — they synthesize new data that was never seen during training.

### 🔴 The Problem It Solves
Before Generative AI, computers could only **recognize** or **classify** content (e.g., "this is a cat" or "this email is spam").
They could not **create** content from scratch.
Generative AI solves this by allowing machines to:
...
#### 📖 Theoretical Definition
> A **Vector Database** is a specialized database designed to store and query high-dimensional numerical representations (called **embeddings**) of data (text, images, audio).
> Embeddings are generated by ML models (like OpenAI's `text-embedding-ada-002`) that map semantically similar content to **nearby points** in a high-dimensional space.
> Search is done using **Approximate Nearest Neighbour (ANN)** algorithms (e.g., HNSW, IVF) which find close vectors in milliseconds — much faster than comparing every entry.

#### 🔴 The Problem It Solves
| Problem | Without Vector DB | With Vector DB |
|---|---|---|
| Searching documents | Exact keyword match only (`LIKE '%keyword%'`) | Semantic similarity (finds meaning, not just words) |
| Speed | Slow full-table scan | Sub-millisecond ANN search |
| Multi-modal | Text only | Text, images, audio — all in one place |
...
#### 📖 Theoretical Definition
> **LLM Frameworks** are orchestration libraries that abstract the plumbing required to build complex AI applications.
> A single LLM call (prompt → response) is simple. But real products require **chaining** multiple calls, managing **memory**, routing queries to **external tools** (calculators, databases, APIs), and handling **errors**.
> Frameworks like LangChain implement the **"chain of thought"** and **"ReAct" (Reason + Act)** patterns to allow LLMs to reason in steps and use tools.
...
### 📖 Theoretical Definition
> A **Python Library** (also called a **package** or **module**) is a pre-built collection of functions, classes, and constants written by domain experts and published to the Python Package Index (PyPI).
> Instead of implementing complex algorithms (matrix multiplication, gradient descent, chart rendering) from scratch, you install the library (`pip install numpy`) and call its functions — abstracting away thousands of lines of optimized, tested code.

### 🔴 The Problem It Solves
| Without Libraries | With Libraries |
|---|---|
| Write sort algorithm from scratch | `sorted(my_list)` — one line |
| Implement matrix multiplication | `np.dot(A, B)` in NumPy |
| Parse a CSV file manually | `pd.read_csv('file.csv')` in Pandas |
| Build a neural network from scratch | `model = Sequential([...])` in TensorFlow/Keras |
...
### 📖 Theoretical Definition
> **Natural Language Processing (NLP)** is the subfield of AI concerned with enabling computers to **understand, interpret, and generate** human language.
> The core challenge: computers process **numbers**, not language. NLP bridges this gap by representing words and sentences as vectors (embeddings) in a mathematical space where similar meanings map to similar positions.
> Early NLP used hand-crafted rules. Modern NLP uses **deep learning** (especially Transformers) to learn language patterns from billions of text examples.

### 🔴 The Problem It Solves
| Problem | Description |
|---|---|
| **Ambiguity** | "Bank" means a financial institution OR a river bank — context matters |
| **Semantics** | "Car" and "automobile" mean the same, but are different words |
| **Long-range dependencies** | The word at position 1 can determine the meaning of the word at position 100 |
| **Languages** | Billions of humans communicate in 7,000+ languages — NLP bridges them |
...
### 📖 Theoretical Definition
> **Prompt Templating** is the practice of separating the static **instruction structure** of a prompt from the dynamic **variable data** that changes per request.
> This is analogous to **SQL prepared statements** (e.g., `SELECT * FROM users WHERE id = ?`) — the query structure is fixed; only the parameter changes.
> At runtime, variables (row data, user input, document text) are injected into the template using Python's `.format()` method or f-strings.

### 🔴 The Problem It Solves
| Problem | Without Templates | With Templates |
|---|---|---|
| Processing 10,000 rows | Manually write 10,000 different prompts | Write 1 template; loop fills it per row |
| Consistency | Each prompt is written differently | Every prompt follows the exact same structure |
| Maintainability | Change the prompt → update thousands of strings | Change the template once → all prompts update |
| Errors | Copy-paste mistakes in each prompt | Template is validated once |
...
> This pattern is a simplified form of **Retrieval-Augmented Generation (RAG)**.
> **RAG** = Instead of relying on the LLM's internal (pre-trained) knowledge, you **retrieve** the relevant document and **augment** the LLM's prompt with it — forcing it to reason over *your* data.
> The LLM acts as a **reasoning engine**, not a knowledge store. The document is the knowledge store.

### 🔴 The Problem It Solves
| Problem | Consequence |
|---|---|
| GPT training data has a cutoff (~2021) | Cannot answer questions about recent events |
| GPT doesn't know your company's internal data | Cannot answer HR policy, financial, or product questions |
| Reading a 100-page PDF manually takes hours | Analysis is slow, prone to error, and not scalable |
...
> **Few-Shot Prompting** is a technique that leverages a property of large language models called **In-Context Learning (ICL)**.
> During training, LLMs saw billions of pattern-completion examples. ICL exploits this by embedding **demonstration examples** (input-output pairs) directly inside the prompt — conditioning the model's behavior without changing its weights.
> Formally:
...
### 🔴 The Problem It Solves
| Problem | Without Few-Shot | With Few-Shot |
|---|---|---|
| GPT gives answers directly | Student doesn't learn | GPT asks guiding questions (tutor behavior) |
| Inconsistent output format | Sometimes JSON, sometimes prose | Examples condition consistent JSON/format |
| Generic tone | GPT sounds robotic | Examples demonstrate domain-specific language |
| No stateful behavior | GPT doesn't build on prior turns | Conversation history teaches multi-turn behavior |
...
### 📖 Theoretical Definition
> **Rate Limiting** is a server-side constraint that throttles the number of API requests a client can make within a time window (e.g., 60 requests/minute for a free tier).
> **Exponential Backoff** is a standard error-handling algorithm where the client, after receiving a rate-limit error (HTTP 429), waits for an **exponentially increasing delay** before retrying — preventing a "thundering herd" of simultaneous retries from overwhelming the server further.
> The pattern is defined in **RFC 2616** and widely used in distributed systems (AWS SDKs, GCP clients, HTTP clients).

### 🔴 The Problem It Solves
| Scenario | Problem | Consequence |
|---|---|---|
| 100 users call your chatbot simultaneously | 100 OpenAI API calls fire at once | 90+ fail with `429 Too Many Requests` |
| Naive retry (retry immediately) | All 100 retry at the same time | Server gets hammered again — cascading failure |
| No retry at all | First failure = permanent user error | Poor user experience |
| Exponential backoff | Spread retries over 1s, 2s, 4s, 8s... | Server recovers; most requests eventually succeed |
...
### 📖 Theoretical Definition
> **Open-Source LLMs** are language models whose **model weights** (the billions of learned parameters) are publicly released, allowing anyone to download, run, fine-tune, or modify them without cost or API dependency.
> **Hugging Face** is the central repository and toolkit for open-source ML models. It provides:
...
> - **Model Hub:** Over 500,000 pre-trained models in one searchable registry
> LLMs like **LLaMA 3** use a **decoder-only Transformer** architecture. During inference, they predict the next token in a sequence, one at a time (autoregressive generation), until an end-of-sequence token is produced.

### 🔴 The Problem It Solves
| Problem with Closed APIs (OpenAI) | How Open-Source Solves It |
|---|---|
| **Cost:** Pay per token — expensive at scale | Run locally for **free**, unlimited calls |
| **Privacy:** Your data leaves your server | Model runs **on your hardware** — data never sent externally |
| **Dependency:** OpenAI can deprecate a model overnight | You own and host the weights permanently |
| **Customization:** Fine-tuning is restricted or expensive | Full access to weights — fine-tune on your own data freely |
| **Compliance:** GDPR / HIPAA may disallow sending data to a third party | Self-hosted model = full data sovereignty |
